...
LINES TERMINATED BY '\n'
IGNORE 1 ROWS
-- creating variables for two columns, Order_Amount & Delivery_Time, having datetime data(but currently set as string type), as placeholders in table structure
(OrderID,Customer_Name,Restaurant_Name,@OrderDate,@DeliveryTime,Delivery_Address,City,Delivery_Status,Order_Amount,Delivery_Agent)
-- formatting Order_Amount & Delivery_Time to desired datetime format
SET 
Order_Date = str_to_date(@OrderDate,'%d-%m-%Y %H:%i'),
Delivery_Time = str_to_date(@DeliveryTime,'%d-%m-%Y %H:%i');
...
SELECT 
    ...
    DATEDIFF(Delivery_Time, CURRENT_TIMESTAMP()) AS Date_Diff
FROM
    swiggy_orders;
...
SELECT 
    ...
    DATE_ADD(Delivery_Time,
        INTERVAL 45 MINUTE) AS Updated_Time
FROM
    swiggy_orders;
...
SELECT 
    ...
    CONVERT_TZ(Delivery_Time, '+00:00', '+05:30') AS Asia_Kolkata_TZ
FROM
    swiggy_orders;
...
FROM 
	swiggy_orders
WHERE 
	Order_Date BETWEEN str_to_date(concat(YEAR(CONVERT_TZ(Delivery_Time, '+5:30', '-4:00')),'-03-10'), '%Y-%m-%d')
	AND str_to_date(concat(YEAR(CONVERT_TZ(Delivery_Time, '+5:30', '-4:00')),'-11-03'), '%Y-%m-%d');
...
GROUP BY OrderID,Order_Date HAVING DAY(Order_Date) = 29 AND MONTH(Order_Date) = 'February' ;
...
SELECT 
    ...
    TIMEDIFF(Order_Date,convert_tz(Order_Date,'+5:30','-7:00')) AS CurrTime_Diff_Kol_LA
FROM
    swiggy_orders;
